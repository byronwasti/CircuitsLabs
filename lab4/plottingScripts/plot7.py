...
def plot_IvI(FILENAME, label):
    ...
    log_i_x = np.log(i_x[100:150])
    log_i_y = np.log(i_y[100:150])
...
if __name__ == "__main__":
    fit1 = plot_IvI("../data/experiment3_y_2K_1.csv", "$2K\Omega$")
    fit2 = plot_IvI("../data/experiment3_y_20K_1.csv", "$20K\Omega$")
    fit3 = plot_IvI("../data/experiment3_y_200K_1.csv", "$200K\Omega$")

    plt.text(1.5e-8, 1.5e-6, '$I_z$ = $I_x^\\alpha$ / $\\beta$\n$\\beta$ = $I_y$\n$\\alpha = %f$, $\\beta = %eA$\n$\\alpha = %f$, $\\beta = %eA$\n$\\alpha = %f$, $\\beta = %eA$' %(fit1[0], 1 / np.exp(fit1[1]), fit2[0], 1 / np.exp(fit2[1]), fit3[0], 1 / np.exp(fit3[1])), fontsize=16)
